...
beforeAll(async () => {
    ...
    admin.initializeApp({ projectId: 'e2e-test' });
    ...
    ({ match_finalizer } = await Promise.resolve().then(() => __importStar(require('../src/match_finalizer'))));
});
afterAll(async () => {
...
it('happy path – pending ticket becomes won and wallet credited', async () => {
    ...
    const msg = { data: Buffer.from(JSON.stringify({ job: 'result-poller' })).toString('base64') };
