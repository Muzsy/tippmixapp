    ...
    if (fixtureIdFromMsg) {
      ...
      for (let i = 0; i < ticketSnaps.length; i++) {
        ...
        try {
          ...
          const updatedTips = tipsRaw.map((orig: any, ti: number) =>
            ti === idx
              ? { ...orig, result: newStatus, fixtureId: Number(fid) }
              : orig,
          );
          await snap.ref.update({ tips: updatedTips } as any);
        } catch (e) {
        ...
        const shouldFinalize =
          tipResults.some((t: any) => t.result === 'lost') ||
          !tipResults.some((t: any) => t.result === 'pending');
        if (shouldFinalize) {
