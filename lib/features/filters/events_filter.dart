...
class EventsFilter {
  ...
  static List<OddsEvent> apply(List<OddsEvent> source, EventsFilter f) {
    ...
    return source.where((e) {
      ...
      final okCountry = f.country == null ? true : e.countryName == f.country;
      final okLeague = f.league == null ? true : e.leagueName == f.league;
      return okDate && okCountry && okLeague;
    }).toList();
  }

  static List<String> countriesOf(List<OddsEvent> source) =>
      source.map((e) => e.countryName).whereType<String>().toSet().toList()
        ..sort();
  static List<String> leaguesOf(List<OddsEvent> source, {String? country}) =>
      source
          .where(
            (e) =>
                country == null || country.isEmpty || e.countryName == country,
          )
          .map((e) => e.leagueName)
          .whereType<String>()
          .toSet()
